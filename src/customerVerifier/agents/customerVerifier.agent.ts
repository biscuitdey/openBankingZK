...
export class CustomerVerifierAgent {
  ...
  async throwIfSignatureVerificationFails(
    message: string,
    signature: string,
    publicKey: string,
  ): Promise<void> {
    if (
      !(await this.verifyEddsaSignatureAgainstPublicKey(
        message,
        signature,
        publicKey,
      ))
  ...
  private constructCircuitPathsFromWorkstepName(name: string): {
    ...
    const snakeCaseWorkstepName = name; //this.convertStringToSnakeCase(name);
